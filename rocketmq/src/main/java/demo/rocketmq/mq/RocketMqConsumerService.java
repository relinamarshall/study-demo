...
/**
 * RocketMqTestListener
 *
 * @author Wenzhou
 * @since 2023/5/4 15:22
 */
@Slf4j
@Component
public class RocketMqConsumerService {
     ...
     */
    @Service
    @RocketMQMessageListener(topic = RocketMQ.TOPIC, selectorExpression = "tag1", consumerGroup = "Con_Group_One")
     ...
     * <p>
     * 不然生产者发送一条消息，这两个都会去消费，如果类型不同会有一个报错，所以实际运用中最好加上tag，写这只是让你看知道就
     */
    @Service
    @RocketMQMessageListener(topic = RocketMQ.TOPIC, consumerGroup = "Con_Group_Two")
     ...
     */
    @Service
    @RocketMQMessageListener(topic = RocketMQ.TOPIC, selectorExpression = "tag2", consumerGroup = "Con_Group_Three")
