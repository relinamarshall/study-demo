...
import demo.neo4j.payload.ClassmateInfoGroupByLesson;
import demo.neo4j.payload.TeacherStudent;
import demo.neo4j.repository.ClassRepository;
import demo.neo4j.repository.LessonRepository;
import demo.neo4j.repository.StudentRepository;
import demo.neo4j.repository.TeacherRepository;
...
public class NeoService {
    ...
    public void initData() {
        ...
        // 初始化学生
        List<Student> threeClass = Lists.newArrayList(Student.of("漩涡鸣人",
                        Lists.newArrayList(tishu, shoulijian, luoxuanwan, xianshu), seven),
        ...
        List<Student> sevenClass = Lists.newArrayList(Student.of("李洛克",
                Lists.newArrayList(tishu), three), Student.of("日向宁次",
                Lists.newArrayList(tishu), three), Student.of("天天",
                Lists.newArrayList(tishu), three));
    ...
    public List<Lesson> findLessonsFromStudent(String studentName, int depth) {
        ...
        studentRepo.findByName(studentName, depth).ifPresent(student ->
    ...
    public Map<String, List<Student>> findClassmatesGroupByLesson() {
        ...
        groupByLesson.forEach(classmateInfoGroupByLesson ->
                result.put(classmateInfoGroupByLesson.getLessonName(), classmateInfoGroupByLesson.getStudents()));
    ...
    public Map<String, Set<Student>> findTeacherStudent() {
        ...
        teacherStudentByClass.forEach(teacherStudent ->
                result.put(teacherStudent.getTeacherName(), Sets.newHashSet(teacherStudent.getStudents())));

        teacherStudentByLesson.forEach(teacherStudent ->
                result.put(teacherStudent.getTeacherName(), Sets.newHashSet(teacherStudent.getStudents())));
