...
import demo.neo4j.service.NeoService;
...
public class Neo4jTest extends Neo4jApplicationTest {
    ...
    public void testFindClassmates() {
        ...
        classmates.forEach((k, v) -> log.info("因为一起上了【{}】这门课，成为同学关系的有：{}", k,
                JSONUtil.toJsonStr(v.stream().map(Student::getName).collect(Collectors.toList()))));
    }
    ...
    public void testFindTeacherStudent() {
        ...
        teacherStudent.forEach((k, v) -> log.info("【{}】教的学生有 {}", k,
                JSONUtil.toJsonStr(v.stream().map(Student::getName).collect(Collectors.toList()))));
    }
}
